...
private static async Task<object?> DeserializeResponse(Type responseType, HttpResponseMessage httpResponseMessage)
{
    ...
    var deserialize = RaitSerializationConfig.DeserializeFunction ??
                      ((content, type) => content.ReadFromJsonAsync(type, RaitSerializationConfig.SerializationOptions));
...
private static HttpStatusCode? GetExpectedStatus(Type t)
{
    if (t.IsGenericType)
    {
        ...
        if (def == typeof(NotFound<>)) return HttpStatusCode.NotFound;
    ...
    Func<HttpContent, Type, Task<object?>> deserialize)
{
    ...
    // Ok<T>
    if (IsGenericTypeDefinition(resultType, typeof(Ok<>)) || (status == HttpStatusCode.OK && IsGenericTypeDefinition(resultType, typeof(Ok<>))))
    {
    ...
    // Created<T>
    if (IsGenericTypeDefinition(resultType, typeof(Created<>)) || (status == HttpStatusCode.Created && IsGenericTypeDefinition(resultType, typeof(Created<>))))
    {
        var innerType = resultType.GetGenericArguments()[0];
        var value = await DeserializeBodyWithFallback(response, innerType, deserialize);
        var location = GetLocationHeader(response) ?? "/";
        // Try TypedResults.Created(string, T)
        return InvokeTypedResultsGeneric("Created", innerType, [location, value]);
    }

    // NotFound<T> and NotFound (non-generic)
...
private static object InvokeTypedResultsGeneric(string methodName, Type genericArg, object[] args)
{
    ...
    throw new MissingMethodException($"RAIT: Could not find TypedResults.{methodName}<{genericArg.Name}> overload for provided arguments.");
}
...
private static object InvokeTypedResults(string methodName, Type[] genericArgs, object[] args)
{
    ...
    throw new MissingMethodException($"RAIT: Could not find TypedResults.{methodName} overload for provided arguments.");
}
